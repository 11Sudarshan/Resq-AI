    ...
    return (
      ...
      >
        <div
          ref={ref}
          className={cn(
            "bg-slate-50 border-slate-200 h-full transition-all duration-300 flex-none",
...
>(({ className, ...props }, ref) => {
  ...
  return (
    ...
    >
      <h2
        className={cn(
          "text-sm text-slate-500 font-semibold tracking-wide whitespace-nowrap",
          isCollapsed
      ...
      <button
        ...
        className={cn(
          `p-1 hover:bg-slate-200 text-slate-500 transition-colors rounded-md cursor-pointer absolute flex items-center justify-center`,
...
>(({ ...props }, ref) => {
  ...
  const handleNewThread = React.useCallback(
    ...
    [startNewThread, refetch, onThreadChange, setSupplies, setMarkers],
  );
  ...
  return (
    <button
      ...
      className={cn(
        "flex items-center rounded-md mb-4 bg-white border border-slate-200 hover:bg-slate-100 text-slate-700 transition-colors cursor-pointer relative shadow-sm",
        isCollapsed ? "p-1 justify-center h-10" : "p-2 gap-2 h-10",
      )}
      title="New thread"
      {...props}
    >
  ...
  return (
    <div ref={ref} className={cn("mb-4 relative", className)} {...props}>
      <button
        ...
        className={cn(
          "p-1 hover:bg-slate-200 rounded-md cursor-pointer absolute left-1/2 -translate-x-1/2",
          isCollapsed
      ...
      >
        ...
        <input
          ref={searchInputRef}
          type="text"
          className="pl-10 pr-4 py-2 w-full text-sm rounded-md bg-white border border-slate-200 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all"
          placeholder="Search..."
          value={searchQuery}
...
>(({ className, ...props }, ref) => {
  ...
  const [deletedThreadIds, setDeletedThreadIds] = React.useState<Set<string>>(new Set());
  ...
  const handleDeleteThread = async (threadToDelete: TamboThread) => {
    if (!confirm("Delete this chat session?")) return;

    setDeletedThreadIds(prev => new Set(prev).add(threadToDelete.id));

    if (currentThread?.id === threadToDelete.id) {
       setSupplies([]);
       setMarkers([]);
       await startNewThread();
    }
  ...
  const filteredThreads = useMemo(() => {
    ...
    return threads.items
      .filter((thread: TamboThread) => !deletedThreadIds.has(thread.id)) 
      .filter((thread: TamboThread) => {
         return (thread.name?.toLowerCase().includes(query) || thread.id.includes(query));
      });
  }, [isCollapsed, threads, searchQuery, deletedThreadIds]);
  ...
  if (isLoading) {
    content = (
      <div
        ref={ref}
        className={cn("text-sm text-slate-400 p-2", className)}
        {...props}
      >
        Loading threads...
      </div>
    ...
    content = (
      <div
        ref={ref}
        className={cn(
          `text-sm text-red-500 p-2 ${isCollapsed ? "opacity-0 max-w-0 overflow-hidden" : "opacity-100"}`,
          className,
        )}
        {...props}
      >
    ...
    content = (
      <div
        ref={ref}
        className={cn(
          `text-sm text-slate-400 p-2 whitespace-nowrap ${isCollapsed ? "opacity-0 max-w-0 overflow-hidden" : "opacity-100"}`,
          className,
        )}
        {...props}
      >
      ...
      <div className="space-y-1">
        {filteredThreads.map((thread: TamboThread) => (
          <div
            ...
            className={cn(
              "p-2 rounded-md cursor-pointer group flex items-center justify-between transition-colors",
              currentThread?.id === thread.id 
                ? "bg-primary/10 border border-primary/20 text-emerald-700 shadow-sm" 
                : "hover:bg-slate-100 text-slate-600",
              editingThread?.id === thread.id ? "bg-white" : "",
            )}
          >
              ...
              {editingThread?.id === thread.id ? (
                ...
                >
                  <input
                    ...
                    className="w-full bg-white border border-primary/30 px-1 text-sm font-medium focus:outline-none rounded-sm"
  ...
  return (
    <DropdownMenu.Root>
      <DropdownMenu.Trigger asChild>
        <button
          className="p-1 hover:bg-slate-200 rounded-md opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer"
          onClick={(e) => e.stopPropagation()}
        >
      ...
      <DropdownMenu.Portal>
        <DropdownMenu.Content
          className="min-w-[160px] text-xs bg-white rounded-md p-1 shadow-md border border-slate-200 z-50"
          sideOffset={5}
          align="end"
        >
          <DropdownMenu.Item
            className="flex items-center gap-2 px-2 py-1.5 text-slate-700 hover:bg-slate-100 rounded-sm cursor-pointer outline-none transition-colors"
            onClick={(e) => {
              e.stopPropagation();
              onRename(thread);
            }}
